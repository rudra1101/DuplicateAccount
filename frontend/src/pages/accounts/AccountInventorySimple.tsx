    ...
    <PageContainer title="Accounts">
      <Stack spacing={3}>
        ...
        <Paper variant="outlined">
          ...
          {tab === 0 ? (
            ...
            <TableContainer>
              <Table size="small">
                <TableHead><TableRow><TableCell>Account</TableCell><TableCell>Application</TableCell><TableCell>Orphan Type</TableCell><TableCell>Correlation Confidence</TableCell><TableCell>Correlation Method</TableCell><TableCell>Status</TableCell><TableCell>Detected</TableCell></TableRow></TableHead>
                <TableBody>
                  {loading ? <TableRow><TableCell colSpan={7} align="center"><CircularProgress size={26} /></TableCell></TableRow> : orphans.length === 0 ? (
                    <TableRow><TableCell colSpan={7} align="center">No orphan accounts found for {selectedIntegration?.name ?? "this source"}.</TableCell></TableRow>
                  ) : orphans.map((finding) => (
                    <TableRow key={finding.id} hover onClick={() => setSelectedOrphan(finding)} sx={{ cursor: "pointer" }}>
                      <TableCell>{orphanLabel(finding)}</TableCell><TableCell>{finding.application}</TableCell><TableCell>{finding.orphanType.replaceAll("_", " ")}</TableCell><TableCell>{finding.confidence}%</TableCell><TableCell>{finding.correlationMethod || "No match"}</TableCell><TableCell>{finding.accountStatus || "—"}</TableCell><TableCell>{new Date(finding.createdAt).toLocaleString()}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      <Drawer anchor="right" open={Boolean(selectedOrphan)} onClose={() => setSelectedOrphan(null)}>
        <Box sx={{ width: 600, p: 3 }}>
          ...
          {selectedOrphan && <Stack spacing={2} sx={{ mt: 2 }}><Typography><strong>Account:</strong> {orphanLabel(selectedOrphan)}</Typography><Typography><strong>Orphan Type:</strong> {selectedOrphan.orphanType.replaceAll("_", " ")}</Typography><Typography><strong>Correlation Confidence:</strong> {selectedOrphan.confidence}%</Typography><Typography><strong>Correlation Method:</strong> {selectedOrphan.correlationMethod || "No identity matched"}</Typography><Typography><strong>Finding Status:</strong> {selectedOrphan.status}</Typography><Typography fontWeight={700}>Correlation Evidence</Typography><Paper variant="outlined" sx={{ p: 2, maxHeight: 600, overflow: "auto" }}>{Object.entries(selectedOrphan.evidence).map(([key, value]) => <Box key={key} sx={{ py: 0.75 }}><Typography variant="caption" color="text.secondary">{key}</Typography><Typography variant="body2" sx={{ whiteSpace: "pre-wrap" }}>{typeof value === "object" ? JSON.stringify(value, null, 2) : String(value ?? "") || "—"}</Typography></Box>)}</Paper></Stack>}
        </Box>
      </Drawer>
    </PageContainer>
  );
