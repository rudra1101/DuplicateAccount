...
function normalizeBackendDate(value: string): string {
  ...
  const hasTimezone =
    isoValue.endsWith("Z") || /[+-]\d{2}:\d{2}$/.test(isoValue);

  return hasTimezone ? isoValue : `${isoValue}Z`;
}
...
const Operations = () => {
  ...
  const [detailsTarget, setDetailsTarget] = useState<OperationExecution | null>(
    null,
  );
  ...
  const loadOperations = useCallback(
    async (showInitialLoader = false) => {
      try {
        ...
        const [summaryResult, operationsResult, integrationsResult] =
          await Promise.all([
            getOperationsSummary(),
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        >
          <SummaryCard
            ...
            icon={<PlayCircleOutlineIcon color="info" />}
          />
        </Grid>
            ...
            <TableBody>
              {executions.map((execution) => (
                ...
                >
                  ...
                  <TableCell>
                    {formatDateTime(execution.startedAt, "Asia/Kolkata")}
                  </TableCell>
                  ...
                  <TableCell align="right">
                    {Number(execution.accountsScanned ?? 0).toLocaleString()}
                  </TableCell>

                  <TableCell align="right">
                    {Number(execution.duplicateGroups ?? 0).toLocaleString()}
                  </TableCell>

                  <TableCell align="right">
                    {Number(execution.duplicateAccounts ?? 0).toLocaleString()}
                  </TableCell>
