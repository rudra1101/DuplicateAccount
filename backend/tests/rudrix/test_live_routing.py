...
def test_system_duplicate_count_routes_to_dashboard(require_live_rudrix, chat_request_factory, db_session):
    response = run_identity_agent(
        db=db_session,
        request=chat_request_factory("How many duplicate accounts do we have?"),
    )
    assert any(t.name == "get_dashboard_summary" for t in response.toolsUsed)
    assert_no_meta_leakage(response.message)
    assert response.message.strip()

def test_candidate_confidence_above_90_routes_to_confidence_breakdown(require_live_rudrix, chat_request_factory, db_session):
    response = run_identity_agent(
        db=db_session,
        request=chat_request_factory("How many duplicate accounts have more than 90% confidence?"),
    )
...
def test_at_least_95_uses_gte(require_live_rudrix, chat_request_factory, db_session):
    response = run_identity_agent(
        db=db_session,
        request=chat_request_factory("How many accounts have at least 95% confidence?"),
    )
    tool = find_tool(response, "get_confidence_breakdown")
    assert tool is not None
...
def test_follow_up_application_wise_keeps_previous_threshold(require_live_rudrix, chat_request_factory, db_session):
    response = run_identity_agent(
        db=db_session,
        request=chat_request_factory(
            ...
            history=[
                ...
                ("assistant", "There are matching duplicate candidate accounts above 90% confidence."),
            ],
        ),
    )
    tool = find_tool(response, "get_confidence_breakdown")
...
def test_group_listing_routes_to_group_search(require_live_rudrix, chat_request_factory, db_session):
    response = run_identity_agent(
        db=db_session,
        request=chat_request_factory("Show top 10 duplicate groups above 95% confidence in SAP."),
    )
...
def test_live_answer_does_not_invent_source_section(require_live_rudrix, chat_request_factory, db_session):
    response = run_identity_agent(
        db=db_session,
        request=chat_request_factory("How many duplicate accounts do we have?"),
    )
