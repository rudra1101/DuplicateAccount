...
def test_stream_emits_status_delta_and_done(require_live_rudrix, chat_request_factory, db_session):
    events = list(run_identity_agent_stream(
        db=db_session,
...
def test_stream_uses_dashboard_tool_for_system_duplicate_count(require_live_rudrix, chat_request_factory, db_session):
    events = list(run_identity_agent_stream(
        db=db_session,
        request=chat_request_factory("How many duplicate accounts do we have?"),
    ))
    done_response = events[-1]["response"]
    assert any(t.name == "get_dashboard_summary" for t in done_response.toolsUsed)
